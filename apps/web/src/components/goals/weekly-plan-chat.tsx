...
function PlanPreviewModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-[0_24px_64px_rgba(30,34,39,0.15)] w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        <div className="px-6 pt-6 pb-4 border-b border-[#E6E1D8]">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-[#4C7CF0]/10">
              <Calendar className="h-5 w-5 text-[#4C7CF0]" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-[#1E2227]">Weekly Plan Preview</h2>
              <p className="text-xs text-[#8C939B]">
                {totalTasks} tasks · ~{totalHours}h total
              </p>
            </div>
          </div>
          {plan.ai_summary && (
            <p className="text-sm text-[#626A73] leading-relaxed">{plan.ai_summary}</p>
          )}
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[0, 1, 2, 3, 4, 5, 6].map((dayIdx) => {
              ...
              return (
                <div key={dayIdx} className="bg-[#F7F5F1] rounded-xl p-4 border border-[#E6E1D8]/50">
                  <p className="text-sm font-semibold text-[#1E2227] mb-2">{DAY_NAMES[dayIdx]}</p>
                  <ul className="space-y-2">
                    {tasks.map((task, i) => (
                      <li key={i} className="text-sm text-[#626A73]">
                        <p className="text-[#1E2227]">{task.title}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          {task.time_slot && (
                            <span className="text-[11px] text-[#8C939B]">{task.time_slot}</span>
                          )}
        ...
        <div className="px-6 py-4 border-t border-[#E6E1D8] flex items-center gap-3">
          <button
            onClick={onConfirm}
            disabled={isSaving}
            className="flex-1 flex items-center justify-center gap-2 bg-[#4D8B6A] text-white text-sm font-medium py-3 rounded-xl hover:bg-[#3D7A5A] active:scale-[0.98] transition-all disabled:opacity-50"
          >
          ...
          <button
            onClick={onEdit}
            disabled={isSaving}
            className="flex items-center justify-center gap-2 text-sm font-medium py-3 px-5 rounded-xl border border-[#D8D1C6] text-[#626A73] hover:bg-[#F1EEE8] active:scale-[0.98] transition-all disabled:opacity-50"
          >
      ...
      <div className="w-96 shrink-0 border-l border-[#E6E1D8] bg-[#F7F5F1] flex flex-col h-[calc(100vh-96px)] sticky top-0">
        ...
        <div className="flex items-center justify-between h-14 px-4 border-b border-[#E6E1D8]">
          <div className="flex items-center gap-2 min-w-0">
            <Sparkles className="h-4 w-4 text-[#4C7CF0] shrink-0" />
            <span className="text-sm font-medium text-[#1E2227] truncate">
        ...
        {/* Input */}
        <div className="border-t border-[#E6E1D8] px-4 py-3">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 text-sm bg-transparent outline-none placeholder:text-[#8C939B] text-[#1E2227] disabled:opacity-50"
            />
            <button
              onClick={handleSend}
              disabled={!inputText.trim() || isStreaming || stage === "saving"}
              className="p-1.5 text-[#4C7CF0] hover:text-[#3F6FE4] transition-colors disabled:opacity-40"
